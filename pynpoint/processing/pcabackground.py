...
class PCABackgroundPreparationModule(ProcessingModule):
    ...
    def __init__(
        ...
        dither: Union[Tuple[int, int, int], Tuple[int, None, Tuple[float, float]]],
        combine: str = "mean",
    ) -> None:
        """
        Parameters
    ...
    def _prepare(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        for i, item in enumerate(nframes):
            if self.m_combine == "mean":
                cube_mean[i,] = np.mean(
                    self.m_image_in_port[count : count + item,], axis=0
                )

            elif self.m_combine == "median":
                cube_mean[i,] = np.median(
                    self.m_image_in_port[count : count + item,], axis=0
                )
        ...
        if self.m_dither[1] is None:
            ...
            star = np.logical_and(
                dither_x == self.m_dither[2][0], dither_y == self.m_dither[2][1]
            )
            ...
            for i in range(
                self.m_dither[2] * self.m_dither[1],
                np.size(nframes),
    ...
    def _separate(
        ...
        def _select_background(i: int) -> np.ndarray:
            ...
            if bg_prev is None and bg_next is not None:
                ...
                background = (bg_prev + bg_next) / 2.0

            else:
                raise ValueError(
                    "Neither previous nor next background frames are found."
                )
        ...
        for i, item in enumerate(nframes):
            progress(
                i, len(nframes), "Preparing PCA background subtraction...", start_time
            )
            ...
            if bg_frames[i]:
                ...
                background_index = np.append(
                    background_index, index[count : count + item]
                )

                if parang is not None:
                    background_parang = np.append(
    ...
    def run(self) -> None:
        ...
        (
            ...
            background_parang,
            background_nframes,
        ) = self._separate(bg_frames, bg_indices, parang, cube_mean)
        ...
        self.m_subtracted_out_port.add_history(
            "PCABackgroundPreparationModule", history
        )
        ...
        if parang is not None:
            self.m_subtracted_out_port.add_attribute(
                "PARANG", star_parang, static=False
            )

        self.m_background_out_port.copy_attributes(self.m_image_in_port)
        self.m_background_out_port.add_history(
            "PCABackgroundPreparationModule", history
        )
        self.m_background_out_port.add_attribute(
            "NFRAMES", background_nframes, static=False
        )
        self.m_background_out_port.add_attribute(
            "INDEX", background_index, static=False
        )

        if parang is not None:
            self.m_background_out_port.add_attribute(
                "PARANG", background_parang, static=False
            )
...
class PCABackgroundSubtractionModule(ProcessingModule):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if "subtract_mean" in kwargs:
            warnings.warn(
                "The 'subtract_mean' parameter has been deprecated. Subtracting of "
    ...
    def run(self) -> None:
        ...
        @typechecked
        def _create_mask(
            radius: float, position: np.ndarray, nimages: int
        ) -> np.ndarray:
            """
            ...
            for i in range(nimages):
                rr_grid = np.sqrt(
                    (xx_grid - cent_x[i]) ** 2 + (yy_grid - cent_y[i]) ** 2
                )
                mask[i,][rr_grid < radius] = 0.0

            return mask

        @typechecked
        def _create_basis(images: np.ndarray, pca_number: int) -> np.ndarray:
            """
        ...
        def _model_background(
            basis: np.ndarray, im_arr: np.ndarray, mask: np.ndarray
        ) -> np.ndarray:
            """
            ...
            @typechecked
            def _dot_product(x_dot: np.ndarray, *p: np.float64) -> np.ndarray:
            ...
            for i in range(im_arr.shape[0]):
                ...
                fitted = curve_fit(
                    _dot_product, basis_reshaped_masked, data_to_fit.reshape(-1), init
                )
        ...
        for i, _ in enumerate(star):
            star[i, :] = locate_star(
                image=self.m_star_in_port[i,] - bg_mean,
                center=None,
        ...
        basis_pca = _create_basis(
            self.m_background_in_port.get_all() - bg_mean, self.m_pca_number
        )
...
class DitheringBackgroundModule(ProcessingModule):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if "mask_planet" in kwargs:
            warnings.warn(
                "The 'mask_planet' parameter has been deprecated.", DeprecationWarning
            )

        if "crop" in kwargs:
            warnings.warn(
                "The 'crop' parameter has been deprecated. The step to crop the "
                "images is no longer optional so crop=True.",
                DeprecationWarning,
            )

        if "prepare" in kwargs:
            warnings.warn(
                "The 'prepare' parameter has been deprecated. The preparation step "
                "is no longer optional so prepare=True.",
                DeprecationWarning,
            )

        if "pca_background" in kwargs:
            warnings.warn(
                "The 'pca_background' parameter has been deprecated. The PCA "
        ...
        if "combine" in kwargs:
            warnings.warn(
                "The 'combine' parameter has been deprecated. To write the mean "
    ...
    def _initialize(self) -> Tuple[int, np.ndarray]:
        if self.m_cubes is None:
            ...
            if self.m_center is None:
                self.m_center = np.copy(dither) + float(npix) / 2.0

                self.m_center = tuple(
                    zip(map(int, self.m_center[:, 0]), map(int, self.m_center[:, 1]))
                )

            else:
                if np.size(dither, axis=0) != np.size(self.m_center, axis=0):
                    raise ValueError(
                        "Number of specified center positions should be equal to the "
    ...
    def run(self) -> None:
        ...
        def _admin_start(
            ...
            star_pos: Union[np.ndarray, np.int64],
        ) -> None:
        ...
        for i, position in enumerate(self.m_center):
            ...
            module = CropImagesModule(
                name_in=f"crop{i}",
                image_in_tag=self.m_image_in_tag,
                image_out_tag=im_out_tag,
                size=self.m_size,
                center=(int(math.ceil(position[0])), int(math.ceil(position[1]))),
            )
        ...
        module._m_output_ports[
            self.m_image_in_tag + "_dither_combine"
        ].del_all_attributes()
        module.run()
