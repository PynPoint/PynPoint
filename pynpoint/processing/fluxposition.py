...
class FakePlanetModule(ProcessingModule):
    ...
    def run(self) -> None:
        ...
        if psf_shape[-2:] != im_shape[-2:]:
            raise ValueError(
                f"The images in '{self.m_image_in_port.tag}' should have the same "
        ...
        history = (
            f"(sep, angle, mag) = ({self.m_position[0]*pixscale:.2f}, "
...
class SimplexMinimizationModule(ProcessingModule):
    ...
    def __init__(
        ...
        if isinstance(pca_number, int):
            ...
            for item in pca_number:
                self.m_res_out_port.append(
                    self.add_output_port(res_out_tag + f"{item:03d}")
                )
                self.m_flux_pos_port.append(
                    self.add_output_port(flux_position_tag + f"{item:03d}")
                )
    ...
    def run(self) -> None:
        ...
        print(
            f"   - Guessed position (x, y) = ({self.m_position[0]:.2f}, "
        ...
        aperture = (
            round(self.m_position[1]),
            round(self.m_position[0]),
        ...
        pos_init = rotate_coordinates(
            center, (self.m_position[1], self.m_position[0]), self.m_extra_rot
        )
        ...
        def _objective(
            ...
            if self.m_offset is None or self.m_offset > 0.0:
                pos_y = arg[1]
                pos_x = arg[2]
            ...
            pos_offset = np.sqrt(
                (pos_x - pos_init[1]) ** 2 + (pos_y - pos_init[0]) ** 2
            )
            ...
            res = np.asarray(
                [
                    ...
                    position[0],
                    sep_ang[0] * pixscale,
                    (sep_ang[1] - self.m_extra_rot) % 360.0,
                    mag,
                    chi_sq,
                ]
            )
            ...
            print(
                f"\rSimplex minimization... {n_components} PC - chi^2 = {chi_sq:.2e}",
                end="",
            )
...
class FalsePositiveModule(ProcessingModule):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if "bounds" in kwargs:
            warnings.warn(
                "The 'bounds' keyword argument has been deprecated. Please use "
                "'offset' instead (e.g. offset=3.0).",
                DeprecationWarning,
            )
    ...
    def run(self) -> None:
        ...
        def _snr_optimize(arg: np.ndarray) -> float:
            ...
            pos_offset = np.sqrt(
                (pos_x - self.m_position[0]) ** 2 + (pos_y - self.m_position[1]) ** 2
            )
        ...
        for j in range(nimages):
            ...
            print(
                f"Image {j+1:03d}/{nimages} -> (x, y) = ({x_pos:.2f}, {y_pos:.2f}), "
...
class AperturePhotometryModule(ProcessingModule):
    ...
    def run(self) -> None:
        ...
        print(
            f"Aperture position (x, y) = ({self.m_position[0]:.1f}, {self.m_position[1]:.1f})"
        )
        print(f"Aperture radius (pixels) = ({self.m_radius:.1f})")

        # Position in CircularAperture is defined as (x, y)
        aperture = CircularAperture(
            (self.m_position[0], self.m_position[1]), self.m_radius
        )
        ...
        print(
            f"Mean flux (counts) = {np.mean(data):.2f} +/- {np.std(data)/np.sqrt(data.size):.2f}"
        )
