...
class PSFpreparationModule(ProcessingModule):
    ...
    def __init__(
        ...
        if resize is not None:
            warnings.warn(
                "The 'resize' parameter has been deprecated. Its value is currently "
    ...
    def run(self) -> None:
        ...
        mask = create_mask(
            (int(im_shape[-2]), int(im_shape[-1])), (self.m_cent_size, self.m_edge_size)
        ).astype(bool)
        ...
        for i in range(frames[:-1].size):
            ...
            progress(
                i,
                len(frames[:-1]),
                "Preparing images for PSF subtraction...",
                start_time,
            )
        ...
        if norms:
            self.m_image_out_port.add_attribute(
                name="norm", value=np.hstack(norms), static=False
            )

        # Save cent_size and edge_size as attributes to the output port
        if self.m_cent_size is not None:
            self.m_image_out_port.add_attribute(
                name="cent_size", value=self.m_cent_size * pixscale, static=True
            )

        if self.m_edge_size is not None:
            self.m_image_out_port.add_attribute(
                name="edge_size", value=self.m_edge_size * pixscale, static=True
            )
...
class AngleInterpolationModule(ProcessingModule):
    ...
    def run(self) -> None:
        ...
        for i, _ in enumerate(parang_start):
            progress(
                i, len(parang_start), "Interpolating parallactic angles...", start_time
            )

            if parang_start[i] < -170.0 and parang_end[i] > 170.0:
                parang_start[i] += 360.0

            elif parang_end[i] < -170.0 and parang_start[i] > 170.0:
                parang_end[i] += 360.0

            if steps[i] == 1:
                new_angles = np.append(
                    new_angles, [(parang_start[i] + parang_end[i]) / 2.0]
                )
            ...
            for item in index_sort[frames[i] : frames[i + 1]]:
                if ndim == 3:
                    self.m_image_out_port.append(
                        self.m_image_in_port[item,], data_dim=3
                    )
...
class AngleCalculationModule(ProcessingModule):
    ...
    @typechecked
    def __init__(self, name_in: str, data_tag: str, instrument: str = "NACO") -> None:
        """
        Parameters
        ----------
        name_in : str
        ...
        if self.m_instrument == "NACO":
            ...
            self.m_O_START = (
                0.3 / 3600.0
            )  # According to SPHERE manual page 90/91 (v102)
            self.m_DIT_DELAY = (
                0.1 / 3600.0
            )  # According to SPHERE manual page 90/91 (v102)
            self.m_ROT = 0.838 / 3600.0  # According to SPHERE manual page 90/91 (v102)

            # rotator offset in degrees
            self.m_rot_offset = 0.0  # no offset here
            ...
            self.m_pupil_offset = (
                -135.99 - 100.48
            )  # According to SPHERE manual page 64 (v102)
            ...
            self.m_ROT = 1.65 / 3600.0  # According to SPHERE manual page 90/91 (v102)
            ...
            raise ValueError(
                "The instrument argument should be set to either 'NACO', "
    ...
    @typechecked
    def _attribute_check(self, ndit: np.ndarray, steps: np.ndarray) -> None:

        if not np.all(ndit == steps):
            warnings.warn(
                "There is a mismatch between the NDIT and NFRAMES values. A frame "
        ...
        if self.m_instrument == "SPHERE/IFS":
            warnings.warn(
                "AngleCalculationModule has not been tested for SPHERE/IFS data."
            )
        ...
        if self.m_instrument in ("SPHERE/IRDIS", "SPHERE/IFS"):
            ...
            if self._m_config_port.get_attribute("RA") != "ESO INS4 DROT2 RA":
                ...
                warnings.warn(
                    "For SPHERE data it is recommended to use the header keyword "
    ...
    def run(self) -> None:
        ...
        exptime = self.m_data_in_port.get_attribute("DIT") / 3600.0
        ...
        if "SPHERE" in self.m_instrument:
            ...
            ra = (tmp_ra_h + tmp_ra_m / 60.0 + tmp_ra_s / 3600.0) * 15.0
            dec = tmp_dec_sign * (tmp_dec_d + tmp_dec_m / 60.0 + tmp_dec_s / 3600.0)
        ...
        for i, tmp_steps in enumerate(steps):
            ...
            # Convert to degrees
            sid_time_arr_deg = sid_time_arr * 15.0
        ...
        if self.m_instrument == "NACO":
            ...
            new_angles_corr = new_angles - (90.0 + (self.m_rot_offset - pupil_pos_arr))
        ...
        indices = np.where(new_angles_corr < -180.0)[0]
        if indices.size > 0:
            new_angles_corr[indices] += 360.0
