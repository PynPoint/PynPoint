...
class TestProcessing:
    ...
    def test_output_port_name(self) -> None:
        ...
        module = FitsReadingModule(
            name_in="read", image_tag="images", input_dir=self.test_dir + "images"
        )
        ...
        assert (
            ...
            == "Tag 'test' of ReadingModule 'read' is already "
            "used."
        )

        module = BadPixelSigmaFilterModule(
            name_in="badpixel", image_in_tag="images", image_out_tag="im_out"
        )
        ...
        assert (
            warning[0].message.args[0]
            == "Tag 'test' of ProcessingModule 'badpixel' is "
            "already used."
        )
    ...
    def test_output_port_set_connection(self) -> None:
        ...
        module = BadPixelSigmaFilterModule(
            name_in="badpixel2", image_in_tag="images", image_out_tag="im_out"
        )
    ...
    def test_apply_function(self) -> None:
        ...
        assert np.sum(data) == pytest.approx(
            0.0071536034905338745, rel=self.limit, abs=0.0
        )
        assert data.shape == (5, 11, 11)

        data = self.pipeline.get_data("im_subtract")
        assert np.sum(data) == pytest.approx(7.047314121155779e-19, rel=0.0, abs=1e-10)
        assert data.shape == (5, 11, 11)

    def test_apply_function_args_none(self) -> None:

        module = TimeNormalizationModule(
            name_in="norm", image_in_tag="images", image_out_tag="im_norm"
        )
        ...
        assert np.mean(data) == pytest.approx(
            2.4012571778516812e-06, rel=self.limit, abs=0.0
        )
        assert data.shape == (5, 11, 11)
    ...
    def test_apply_function_args_none_memory_none(self) -> None:
        ...
        module = TimeNormalizationModule(
            name_in="norm_none", image_in_tag="images", image_out_tag="im_norm"
        )
    ...
    def test_apply_function_same_port(self) -> None:
        ...
        data = self.pipeline.get_data("im_subtract")
        assert np.sum(data) == pytest.approx(0.0, rel=0.0, abs=1e-10)
        assert data.shape == (5, 11, 11)
    ...
    def test_apply_function_args_none_memory_none_same_port(self) -> None:
        ...
        module = TimeNormalizationModule(
            name_in="norm_none_same", image_in_tag="images", image_out_tag="images"
        )
    ...
    def test_apply_function_to_images_memory_none(self) -> None:
        ...
        assert np.mean(data) == pytest.approx(
            1.8259937251367536e-05, rel=self.limit, abs=0.0
        )
