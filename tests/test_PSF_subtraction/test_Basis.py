...
class TestBasis(object):
    ...
    def func4test_overall_same(self, basis, basis_base):
        assert np.array_equal(basis.im_size, basis_base.im_size)
        assert np.allclose(basis.im_norm, basis_base.im_norm, rtol=limit)
        assert np.allclose(basis.im_arr, basis_base.im_arr, rtol=limit)
        assert np.allclose(basis.psf_basis[0], basis_base.psf_basis[0], rtol=limit)
        assert np.allclose(basis.psf_basis[1], basis_base.psf_basis[1], rtol=limit)
        assert np.allclose(basis.psf_basis[2], basis_base.psf_basis[2], rtol=limit)
        assert np.allclose(basis.psf_basis[3], basis_base.psf_basis[3], rtol=limit)
        assert np.allclose(basis.cent_mask, basis_base.cent_mask, rtol=limit)
        assert np.allclose(basis.im_ave, basis_base.im_ave, rtol=limit)
