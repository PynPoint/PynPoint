...
class TestDarkAndFlatCalibration(object):
    ...
    def test_dark_calibration(self):
        ...
        module = DarkCalibrationModule(name_in="dark_multi",
                                       image_in_tag="images",
                                       dark_in_tag="dark",
                                       image_out_tag="dark_cal_multi")

        self.pipeline.add_module(module)
        self.pipeline.run_module("dark_multi")
        ...
        data_multi = self.pipeline.get_data("dark_cal_multi")
    ...
    def test_flat_calibration(self):
        ...
        module = FlatCalibrationModule(name_in="flat_multi",
                                       image_in_tag="dark_cal",
                                       flat_in_tag="flat",
                                       image_out_tag="flat_cal_multi")

        self.pipeline.add_module(module)
        self.pipeline.run_module("flat_multi")
        ...
        data_multi = self.pipeline.get_data("flat_cal_multi")
        assert np.allclose(data, data_multi, rtol=limit, atol=0.)
        assert data.shape == data_multi.shape
