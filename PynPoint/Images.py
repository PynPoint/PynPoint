...
class images(pynpoint_parent):
    ...
    #         coeff_temp = np.array((np.mat(temp_im_arr) * np.mat(basis.psf_basis.reshape(basis.psf_basis.shape[0],-1)).T)) # use matrix multiplication
    #         
    # 
    ...
    #     psf_im = (np.mat(self.psf_coeff[:,0:num]) * np.mat(basis.psf_basis.reshape(basis.psf_basis.shape[0],-1)[0:num,]))
    ...
    #     obj = images.create_whdf5input(file_hdf5,ran_sub=False,**kwargs)
    #     return obj
    #           
    # @staticmethod
...
#     # 
    ...
    #     im_out = np.zeros(shape= self.psf_im_arr.shape)
